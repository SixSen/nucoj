...
create table User
(
	userId varchar(20) primary key, #用户Id设为主键
    email varchar(100) not null, #邮箱号，非空
    uPassword varchar(50) not null, #用户密码，非空
    nick varchar(25) unique not null  #昵称唯一，非空
);
...
(
    ...
    rankRule varchar(2) default 'a' check(rankRule in('a','b','c')) 
		#排名规则（暂定有三种），其约束条件：判断类型是否属于('a','b,'c')的其中一种，否则不能写进数据库，与ContestResultSet表中的score有关,默认以这种方式为排名规则
);
